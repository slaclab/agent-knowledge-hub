...
- [x] `"skills": "./skills"` installs all files in that directory preserving subdir structure
- [x] Install warns when skill does not list `claude-code` in `compatible_platforms`
- [x] `validate` passes on marketplace-format plugins (directory-form skills)
- [x] `create` generates a plugin.json with agents/scripts/MCP sections when requested
- [x] `list` shows agent count and platform info
- [x] Remove correctly cleans up commands and agents (manifest-based)
- [x] Update correctly cleans old commands/agents before reinstalling
- [x] Codex install: plugin files in `~/.akh/plugins/<slug>/`, marketplace.json updated, config.toml entries added
- [x] Codex remove: all Codex-side state cleaned up cleanly (no orphan entries in config.toml or marketplace.json)
- [x] Codex absent: installer degrades gracefully without error
